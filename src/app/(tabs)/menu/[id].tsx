...
const ProductDetailsScreen = () => {
    ...
    const addToCart = () =>{
        console.warn(`You bought ${quantity} ${selectedSize} size pizzas.`)
    }
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.quantityContainer}>
                        <Pressable onPress={() => setQuantity(quantity + 1)}>
                            <FontAwesome size={32} name='plus-square-o' color={colorScheme === 'dark' ? Colors.dark.error : Colors.light.link} />
                        </Pressable>
                        <Text style={[
                            styles.sizeText,
                            {
                        ...
                        <Pressable onPress={() => quantity > 0 ? setQuantity(quantity - 1) : setQuantity(0)}>
                            <FontAwesome size={32} name='minus-square-o' color={colorScheme === 'dark' ? Colors.dark.error : Colors.light.link} />
                        </Pressable>
                    </View>
                    <Text style={[styles.description, { color: colorScheme === 'dark' ? Colors.dark.secondaryText : Colors.light.secondaryText }]}>
                        <Text style={{ fontSize: 28 }}>{product?.description.charAt(0)}</Text>
                        <Text>{product?.description.slice(1)}</Text>
                    </Text>
                </View>
            </ScrollView>
            <Pressable
